        ...
        var findTests = function(error, tests) {
            ...
            for (var index in tests) {
                ...
                if (req.query.draw == "-1") {
                    ...
                    req.collections.tests.updateById(tests[index]._id, {
                        $set: tests[index]
                    }, updateTestStatusCallback);
                }
            }
            //Count number of retuned tests and send response.
    ...
    var findTestCriterias = function(error, criterias) {
        var aggregateTestStat = function(error, stat) {
            ...
            for (var j = 0; j < stat.length; j++) {
                values[j] = {
                    name: stat[j][req.query.graph] == null ? "Others" : stat[j][req.query.graph],
                    value: stat[j].count
                };
            }
...
function findTest(req, res, next) {
    ...
    req.collections.tests.findOne({
        _id: mongoskin.helper.toObjectID(req.params.test_id)
    }, fetchTest);
}
/**
...
/**
 * API path /api/testnames/:report_name/:build_id ---newly added api to fetch the test names of  report with a given buildId
 */
exports.testNames = findTestNamesForReportWithBuildId;
function findTestNamesForReportWithBuildId(req, res, next) {
    var fetchTestNames = function(error, tests) {
        //Handle Error.
        if (error) {
            return next(error);
        } else {
            for (var j = 0; j < tests.length; j++) {
                testNames[j] = tests[j].name;
            }
            return res.send({
                tests: testNames
            });
    ...
    var testNames = {};
    if (validator.isNull(req.params.build_id)) {
        return next(new Error(" Build Id should be defined and it should not be null"));
    }
    if (validator.isNull(req.params.report_name)) {
    ...
    req.collections.tests.find({
        ...
        .toArray(fetchTestNames);
}
/**
 * API path /api/testdescription/:report_name/:build_id ---newly added api to fetch the test descriptions of  report with a given buildId
 */
exports.testDescriptions = findTestDescriptionsForReportWithBuildId;
function findTestDescriptionsForReportWithBuildId(req, res, next) {
    var fetchTestDescriptions = function(error, tests) {
        //Handle Error.
        if (error) {
            return next(error);
        } else {
            for (var j = 0; j < tests.length; j++) {
                testDescriptions[j] = tests[j].description;
            }
            return res.send({
    ...
    req.collections.tests.find({
        ...
        .toArray(fetchTestDescriptions);
}
/**
 * API path //api/testcaseid/fail/:build_id---newly added api to get the failed TestCaseId for a given build
 */
exports.failTestCaseIds = findFailedTestCaseIds;
function findFailedTestCaseIds(req, res, next) {
    var fetchFailedTestCaseId = function(error, tests) {
        //Handle Error.
        if (error) {
            return next(error);
        } else {
            for (var j = 0; j < tests.length; j++) {
                if (tests[j].status == 'fail') {
                    failedTestCaseId[j] = tests[j].name;
                }
            }
            return res.send({
                tests: failedTestCaseId
            });
        }
    };
    var testId = {};
    var failedTestCaseId = {};
    if (validator.isNull(req.params.build_id)) {
        return next(new Error(" Build Id should be defined and it should not be null"));
    }
    req.collections.tests.find({
        ...
        .toArray(fetchFailedTestCaseId);
}
/**
 * API path /api/reportnames/:build_id---newly added api to get the reportnames of all test cases for a build
 */
exports.reportNames = findReportNames;
function findReportNames(req, res, next) {
    var fetchReportNames = function(error, reports) {
        //Handle Error.
        if (error) {
            return next(error);
        } else {
            for (var j = 0; j < reports.length; j++) {
                reportNames[j] = reports[j].name;
            }
            return res.send({
                reports: reportNames
            });
    ...
    req.collections.reports.find({
            build_id: req.params.build_id
        })
        .toArray(fetchReportNames);
}
/**
 * API path /api/tests/:build_id/:report_name/:status---newly added api to get the tests  for a report with given buildId and Status
 */
exports.testsForaBuildWithStatus = findTestsForBuildAndReportWithStatus;
function findTestsForBuildAndReportWithStatus(req, res, next) {
    var fetchTests = function(error, tests) {
        //Handle Error.
        if (error) {
            return next(error);
        } else {
            for (var j = 0; j < tests.length; j++) {
                testObjects[j] = tests[j];
            }
            return res.send({
                tests: testObjects
            });
    ...
    if (validator.isNull(req.params.status)) {
        return next(new Error(" Status should be defined and it should not be null"));
    }
    req.collections.tests.find({
            build_id: req.params.build_id,
            report_name: req.params.report_name,
...
/**
 * API path /api/tests/:build_id/:report_name---newly added api to get the tests  for a report with a given build id
 */
exports.testsForaReport = findTestsForBuildAndReport;
function findTestsForBuildAndReport(req, res, next) {
    var fetchTests = function(error, tests) {
        //Handle Error.
        if (error) {
            return next(error);
...
/**
 * API path /api/:build_id/reportIds---newly added api to get the reportIds for a given build
 */
exports.getReportIds = getReportIdsForBuild;
function getReportIdsForBuild(req, res, next) {
    var getReportIds = function(error, reports) {
        //Handle Error.
        if (error) {
            return next(error);
        } else {
            for (var j = 0; j < reports.length; j++) {
                reportObjects[j] = reports[j].id;
            }
            return res.send({
